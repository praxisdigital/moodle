...
function xmldb_bigbluebuttonbn_upgrade($oldversion = 0) {
	...
	/**
	 * Praxis Core hack
	 * Missing fields in newer versions of BigBlueButton.
	 * Some of our sites even back from 2019 have a version of BBB installed where there was missing field creations
	 * in the install.xml file.
	 */
	if ($oldversion < 2022041906) {

		$bigbluebuttonbn_table = new xmldb_table('bigbluebuttonbn');

		$remotedatatstamp_field = new xmldb_field('remotedatatstamp', XMLDB_TYPE_INTEGER, 9, null, XMLDB_NOTNULL, null, 0, null);
		if (!$dbman->field_exists($bigbluebuttonbn_table, $remotedatatstamp_field)) {
			$dbman->add_field($bigbluebuttonbn_table, $remotedatatstamp_field);
		}

		$completionengagementchats_field = new xmldb_field('completionengagementchats', XMLDB_TYPE_INTEGER, 9, null, XMLDB_NOTNULL, null, 0, null);
		if (!$dbman->field_exists($bigbluebuttonbn_table, $completionengagementchats_field)) {
			$dbman->add_field($bigbluebuttonbn_table, $completionengagementchats_field);
		}

		$completionengagementtalks_field = new xmldb_field('completionengagementtalks', XMLDB_TYPE_INTEGER, 9, null, XMLDB_NOTNULL, null, 0, null);
		if (!$dbman->field_exists($bigbluebuttonbn_table, $completionengagementtalks_field)) {
			$dbman->add_field($bigbluebuttonbn_table, $completionengagementtalks_field);
		}

		$completionengagementraisehand_field = new xmldb_field('completionengagementraisehand', XMLDB_TYPE_INTEGER, 9, null, XMLDB_NOTNULL, null, 0, null);
		if (!$dbman->field_exists($bigbluebuttonbn_table, $completionengagementraisehand_field)) {
			$dbman->add_field($bigbluebuttonbn_table, $completionengagementraisehand_field);
		}

		$completionengagementpollvotes_field = new xmldb_field('completionengagementpollvotes', XMLDB_TYPE_INTEGER, 9, null, XMLDB_NOTNULL, null, 0, null);
		if (!$dbman->field_exists($bigbluebuttonbn_table, $completionengagementpollvotes_field)) {
			$dbman->add_field($bigbluebuttonbn_table, $completionengagementpollvotes_field);
		}

		$completionengagementemojis_field = new xmldb_field('completionengagementemojis', XMLDB_TYPE_INTEGER, 9, null, XMLDB_NOTNULL, null, 0, null);
		if (!$dbman->field_exists($bigbluebuttonbn_table, $completionengagementemojis_field)) {
			$dbman->add_field($bigbluebuttonbn_table, $completionengagementemojis_field);
		}

		// Add index to bigbluebuttonbn_logs (Fix for CONTRIB-8157).
		$bigbluebuttonbn_logs_table = new xmldb_table('bigbluebuttonbn_logs');

		$courseid_index = new xmldb_index('courseid', XMLDB_INDEX_NOTUNIQUE, ['courseid']);
		if(!$dbman->index_exists($bigbluebuttonbn_logs_table, $courseid_index)){
			$dbman->add_index($bigbluebuttonbn_logs_table, $courseid_index);
		}

		$log_index = new xmldb_index('log', XMLDB_INDEX_NOTUNIQUE, ['log']);
		if(!$dbman->index_exists($bigbluebuttonbn_logs_table, $log_index)){
			$dbman->add_index($bigbluebuttonbn_logs_table, $log_index);
		}

		$logrow_index = new xmldb_index('logrow', XMLDB_INDEX_NOTUNIQUE, ['courseid', 'bigbluebuttonbnid', 'userid', 'log']);
		if(!$dbman->index_exists($bigbluebuttonbn_logs_table, $logrow_index)){
			$dbman->add_index($bigbluebuttonbn_logs_table, $logrow_index);
		}
	}
