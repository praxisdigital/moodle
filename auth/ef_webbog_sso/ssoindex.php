...
$auth_config = get_config('auth/ef_webbog_sso');


$webbog_key = $auth_config->ef_webbog_ssokey;
$webbog_iv = $auth_config->ef_webbog_ssoiv;

// Try to prevent searching for sites that allow sign-up.
if (!isset($CFG->additionalhtmlhead)) {
    $CFG->additionalhtmlhead = '';
}
$CFG->additionalhtmlhead .= '<meta name="robots" content="noindex" /> <meta http-equiv="P3P" content=\'CP="IDC DSP COR ADM DEVi TAIi PSA PSD IVAi IVDi CONi HIS OUR IND CNT"\'> ';
...
if ($testsession) {
    if ($testsession == $USER->id) {
        ...
        $errorcode = 1;
        redirect(new moodle_url('/login/cookieerrorpage.php'));
    }
}
...
if ($user !== false or $frm !== false or $errormsg !== '') {
    ...
    $crypt_key = $webbog_key;
    $iv = $webbog_iv;

    $encryptedBundleData = @$_GET['bundleddata'];
    $decryptedBundleData = doDecrypt($encryptedBundleData);
    $bundle = json_decode($decryptedBundleData,true);
    
    $use_timestamp_verification = false;
    $use_coursedateid = false;
    $instance_link = '';
    
    if ($bundle['password']=='')
    {
        $bundle['password']='!Itai100';
    }
    if (strlen($bundle['instancetype']) >= 1 && $bundle['instanceid'])
    {
        if ($bundle['instancetype'] == "quiz")
        {
            $instance_link = "/mod/quiz/view.php?id=".$bundle['instanceid'];
        }
        else if ($bundle['instancetype'] == "course")
        {
            $instance_link = "/course/view.php?id=".$bundle[instanceid];
        }
    }
    
    if ($use_timestamp_verification)
    {
        $time1hago = strtotime('-1 hour');
        $timestampdate = strtotime($bundle['timestamp']);

        if ($timestampdate > $time1hago) {
            if ((strlen($bundle['username'])>=1) && (strlen($bundle['password'])>=1)){
                $frm->username = $bundle['username'];
                $frm->password = $bundle['password'];
            }

            if ($use_coursedateid)
            {
                // do something with afviklingsid
                $courseDateId = $bundle['coursedateid'];
            }

            $SESSION->wantsurl=$CFG->wwwroot.$instance_link;
        } 
        else {
        ...
        if ((strlen($bundle['username'])>=1) && (strlen($bundle['password'])>=1)){				
				if (isset($USER->username)){
					if ($USER->username != $bundle['username'])
					{
						$isLoggedIn = false;
					}					
				}				
        ...
        $SESSION->wantsurl=$CFG->wwwroot.$instance_link.$bundle['additionalParameters'];        
    }
	/*end itai section*/
}
...
if ($frm and isset($frm->username)) {                             // Login WITH cookies
    ...
    /// Currently supported only for ldap-authentication module
        ...
        redirect(new moodle_url('/auth/ef_webbog_sso/ssoindex.php', array('testsession'=>$USER->id)));
...
{ 
    ...
    $test = strtr($decrypt, '-_.', '+/=');
    $decoded = base64_decode($test); 
    $decrypted = mcrypt_decrypt(MCRYPT_RIJNDAEL_256, $crypt_key, $decoded, MCRYPT_MODE_CBC, $iv);
    $temp = str_replace("\0", '', $decrypted);
